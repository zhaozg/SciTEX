...
------------------------------------------------------
  Note: Require gui.dll <http://scite-ru.googlecode.com/hg/lualib/gui/>
	 ...
	 abbrevlist.lua (function scite_InsertAbbreviation)
...
-- Common.lua
dofile (props["SciteUserHome"].."/additions/tools/Common.lua")
...
function FileMan_FileRename()
	local filename = FileMan_GetSelectedItem()
	if filename == '' or filename == '..' then return end
	local filename_new = gui.prompt_value("Rename To:", filename)
	if filename_new == nil or filename_new:match('[\\/:|*?"<>]') then return end
...
function FileMan_FileExec(params)
	...
	-- Other
		...
		local cmd = current_path..filename..params
		print('open',cmd)
	end
end
...
--^------- common patterns -------^--
	...
	do --v------- Lua -------v--
		...
		local ls_closeeq = lpeg.Cmt(ls_close * lpeg.Cb("init"), function (s, i, a, b) return a == b end)
		local LUALONGSTR = ls_open * (lpeg.P(1) - ls_closeeq)^0 * ls_close / 1
...
local function Functions_ListFILL()
	...
	for i=#table_functions,1,-1 do
		if (type(table_functions[i])=='string') then
...
local function OnSwitch()
	...
	_,line_count = pcall(function() return editor.LineCount end)
	if line_count then
...
scite_OnOpen(function ()
	...
	print('editor.MouseDwellTime',editor.MouseDwellTime)
end)
