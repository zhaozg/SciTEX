...
--------------------------------------------------------
-- load and run file in props["SciteDefaultHome"] that launch SciTEX

function runfile(file)
	dofile(props["SciteUserHome"]..file:gsub("/","\\"))
end

runfile('\\additions\\extman\\extman.lua')

--runfile ("\\additions\\tools\\URL_detect.lua")
--------------------------------------------------------
...
local function StringLower(s, cp)
	if not cp then cp = tonumber(props["editor.code.page"]) end
	if cp ~= 65001 and type(s)=='string' then s = s:to_utf8(cp) end
	--s = s:lower()
