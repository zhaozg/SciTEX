...
int do_message(lua_State* L)
{
  ...
  int type = 0;
  if (EQ(kind, L"message")) type = 0;
  else if (EQ(kind, L"warning")) type = 1;
  else if (EQ(kind, L"error")) type = 2;
  else if (EQ(kind, L"query")) type = 3;
...
int window_add(lua_State* L)
{
  ...
  else if (EQ(align, L"bottom"))
  {
    child->align(alBottom, sz);
  }
  else if (EQ(align, L"left"))
  {
    child->align(alLeft, sz);
  }
  else if (EQ(align, L"right"))
  {
