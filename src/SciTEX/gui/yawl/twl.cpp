...
//--------------------------------------------
{
  GetDlgItemText(m_hwnd, id, str, sz);
  return str;
}
...
bool TEventWindow::command(int, int)
{
  return true;
}
bool TEventWindow::sys_command(int)
{
  return false;
}
...
int  TEventWindow::handle_user(long lparam, long wparam)
{
  return 0;
}
...
{
  if (modeless()) return (bool) handle();
  else return m_ret;
}
