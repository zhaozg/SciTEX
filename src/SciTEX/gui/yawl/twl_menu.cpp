...
// if (item.id == -1) item.id = last_id++;
  ...
  if (astr)
  {
    ...
    else if (EQ(vkey, L"UP"))     key = VK_UP;
    else if (EQ(vkey, L"DOWN"))   key = VK_DOWN;
    else if (EQ(vkey, L"RIGHT"))  key = VK_RIGHT;
    else if (EQ(vkey, L"LEFT"))   key = VK_LEFT;
    else if (EQ(vkey, L"DELETE")) key = VK_DELETE;
    else
    {
      //accl.fVirt = 0;
      key = (int)vkey[0];
    ...
    if (ckey != NULL)
    {
      if (EQ(ckey, L"CTRL")) accl.fVirt |= FCONTROL;
      else if (EQ(ckey, L"ALT")) accl.fVirt |= FALT;
      else if (EQ(ckey, L"SHIFT")) accl.fVirt |= FSHIFT;
    }
